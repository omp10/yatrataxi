...
const getPortalHomeRoute = (role = "") => {
  const normalizedRole = normalizeDriverPortalRole(role);

  if (normalizedRole === "owner") {
    return "/taxi/owner/home";
  }
...
const RegistrationStatus = () => {
  ...
  useEffect(() => {
    if (location.state?.role) {
      const normalizedRole = normalizeDriverPortalRole(location.state.role);
    ...
    const checkApproval = async () => {
      ...
      const authenticatedRole = normalizeDriverPortalRole(
        getAuthenticatedDriverRole() || getStoredDriverRole() || location.state?.role,
      );
      if (
        authenticatedRole === "bus_driver" ||
        authenticatedRole === "service_center" ||
        authenticatedRole === "service_center_staff"
      ) {
        navigate(getPortalHomeRoute(authenticatedRole), { replace: true });
        requestInFlightRef.current = false;
        return;
      }
      ...
      try {
        ...
        if (isApproved) {
          clearDriverRegistrationSession();
          const normalizedRole = normalizeDriverPortalRole(
            getStoredDriverRole() || location.state?.role || "driver",
          );

          navigate(getPortalHomeRoute(normalizedRole), { replace: true });
          requestInFlightRef.current = false;
          return;
        }
            ...
            <div className="space-y-3">
                ...
                <p className="mx-auto max-w-[28ch] text-[15px] font-bold leading-relaxed text-slate-500 opacity-80">
                    {rejectedDocs.length > 0 
                        ...
                        : statusMessage || "Our team is currently performing a manual audit of your profile."}
                </p>
            </div>
        </section>
