...
const transactionLabel = (type = '', isOwnerWallet = false) => {
    const labels = isOwnerWallet
        ? {
            ride_earning: 'Fleet online earning',
            commission_deduction: 'Fleet cash commission',
            top_up: 'Owner wallet top-up',
            adjustment: 'Owner wallet adjustment',
        }
        : {
...
const transactionHint = (tx = {}, isOwnerWallet = false) => {
    ...
    if (tx.type === 'commission_deduction') {
        return isOwnerWallet
            ? `Fleet COD ride${fare ? ` of ${money(fare)}` : ''}${commission ? `, owner commission ${money(commission)}` : ''}`
            : `COD ride${fare ? ` of ${money(fare)}` : ''}${commission ? `, admin commission ${money(commission)}` : ''}`;
    }

    if (tx.type === 'ride_earning') {
        return isOwnerWallet
            ? `${payment === 'online' ? 'Online' : 'Fleet'} payout after owner commission`
            : `${payment === 'online' ? 'Online' : 'Ride'} payout after admin commission`;
    }
...
const DriverWallet = () => {
    ...
    const isOwnerWallet = location.pathname.startsWith('/taxi/owner');
    const walletRoleLabel = isOwnerWallet ? 'Owner' : 'Driver';
    ...
    const loadWallet = useCallback(async ({ quiet = false } = {}) => {
        ...
        try {
            ...
            if (!token) {
                setLoading(false);
                setRefreshing(false);
                navigate(isOwnerWallet ? '/taxi/owner/login' : '/taxi/driver/login', { replace: true });
                return;
            }
            ...
            const [walletResponse, profileResponse] = await Promise.all([
                ...
                isOwnerWallet ? Promise.resolve(null) : api.get('/drivers/me', authConfig).catch(() => null),
            ]);
            ...
            setDriverProfile({
                salary: isOwnerWallet ? 0 : toNumber(profile.salary, 0),
                isOwnerManagedDriver: isOwnerWallet ? false : isOwnerManagedDriverProfile(profile),
            });
        } catch (requestError) {
    ...
    }, [isOwnerWallet, navigate]);

    useEffect(() => {
        loadWallet({ quiet: true });

        const token = getLocalDriverToken();
        const socket = token ? socketService.connect({ role: isOwnerWallet ? 'owner' : 'driver' }) : null;
        ...
        if (socket) socketService.on(isOwnerWallet ? 'owner:wallet:updated' : 'driver:wallet:updated', onWalletUpdated);

        return () => {
            socketService.off(isOwnerWallet ? 'owner:wallet:updated' : 'driver:wallet:updated', onWalletUpdated);
        };
    }, [isOwnerWallet, loadWallet]);
    ...
    const rules = useMemo(() => {
        const minimumBalance = toNumber(
            ...
            toNumber(
                isOwnerWallet
                    ? settings.owner_wallet_minimum_amount_to_get_an_order
                    : settings.driver_wallet_minimum_amount_to_get_an_order,
                0,
            ),
        );
        ...
        const walletEnabled = wallet.isWalletEnabled ?? isEnabled(
            isOwnerWallet ? settings.show_wallet_feature_for_owner : settings.show_wallet_feature_for_driver,
            true,
        );
        const transferEnabled = wallet.isTransferEnabled ?? isEnabled(
            isOwnerWallet ? settings.enable_wallet_transfer_owner : settings.enable_wallet_transfer_driver,
            true,
        );
        const canReceiveOrders = isOwnerWallet ? walletEnabled : walletEnabled && !wallet.isBlocked && availableForOrders > 0;
    ...
    const walletSummary = useMemo(() => {
        ...
        const totalAppEarnings = transactions
            ...
            .reduce((sum, tx) => {
                ...
                return source === 'driver_incentive' || source === 'owner_adjustment' ? sum + Math.max(amount, 0) : sum;
            }, 0);
    ...
    const handleTopUp = async () => {
        ...
        try {
            ...
            if (!supportsWalletTopUp || !canTopUpWallet) {
                throw new Error(`${walletTopUpGatewayLabel} is enabled by admin, but ${walletRoleLabel.toLowerCase()} wallet top-up is not implemented for it yet.`);
            }
    ...
    const walletIntro = isOwnerWallet
        ? 'Fleet commission and owner earnings'
            ...
            <div className="mx-auto max-w-md">
                <header className="mb-4 flex items-center justify-between">
                    ...
                    <div className="text-center">
                        <h1 className="text-lg font-black tracking-tight">{walletRoleLabel} wallet</h1>
                ...
                {loading ? (
                    ...
                    <main className="space-y-4">
                        <section className="overflow-hidden rounded-[2rem] bg-[#101521] p-5 text-white shadow-xl">
                            ...
                            <div className={`mt-5 grid gap-3 ${driverProfile.isOwnerManagedDriver || isOwnerWallet ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-2'}`}>
                                <div className="rounded-2xl bg-white/10 p-3">
                                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-white/45">
                                        {isOwnerWallet ? 'Minimum wallet' : 'Minimum needed'}
                                    </p>
                                    <p className="mt-1 text-lg font-black">{money(rules.minimumBalance)}</p>
                                </div>
                                <div className="rounded-2xl bg-white/10 p-3">
                                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-white/45">
                                        {isOwnerWallet ? 'Available balance' : 'Available cash limit'}
                                    </p>
                                    <p className={`mt-1 text-lg font-black ${rules.availableForOrders > 0 ? 'text-emerald-200' : 'text-amber-200'}`}>
                                        {money(isOwnerWallet ? wallet.balance : rules.availableForOrders)}
                                    </p>
                                </div>
                            </div>
                        </section>
                        ...
                        {activePaymentGateway && !canTopUpWallet && (
                            <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm font-bold text-amber-700">
                                {walletTopUpGatewayLabel} is active, but {walletRoleLabel.toLowerCase()} wallet top-up is not available for it yet.
                            </div>
                        )}
                        ...
                        <section className="rounded-[1.7rem] bg-white p-4 shadow-sm">
                            <div className="mb-3 flex items-center gap-2">
                                ...
                                <h3 className="text-sm font-black text-slate-950">
                                    {isOwnerWallet ? 'Owner wallet guide' : driverProfile.isOwnerManagedDriver ? 'Wallet activity guide' : 'How it reflects'}
                                </h3>
                            </div>
                            <div className="grid gap-2">
                                <div className="rounded-2xl bg-slate-50 p-3">
                                    <p className="text-sm font-black text-slate-900">
                                        {isOwnerWallet ? 'Fleet cash / COD ride' : driverProfile.isOwnerManagedDriver ? 'Monthly salary' : 'Cash / COD ride'}
                                    </p>
                                    <p className="mt-1 text-xs font-bold leading-relaxed text-slate-500">
                                        {isOwnerWallet
                                            ? 'Fleet driver collects the full cash fare. Owner wallet deducts only the owner commission configured for that vehicle.'
                                            : driverProfile.isOwnerManagedDriver
                                ...
                                <div className="rounded-2xl bg-slate-50 p-3">
                                    <p className="text-sm font-black text-slate-900">
                                        {isOwnerWallet ? 'Fleet online ride' : driverProfile.isOwnerManagedDriver ? 'Wallet balance' : 'Online ride'}
                                    </p>
                                    <p className="mt-1 text-xs font-bold leading-relaxed text-slate-500">
                                        {isOwnerWallet
                                            ? 'Platform receives the fare. Owner wallet credits owner earning after the configured owner commission.'
                                            : driverProfile.isOwnerManagedDriver
                        ...
                        <section className="grid grid-cols-1 gap-3">
                            ...
                            <StatPill label={isOwnerWallet ? 'Owner earnings' : `${appName} earnings`} value={money(walletSummary.totalAppEarnings)} tone="good" />
                            {!isOwnerWallet ? <StatPill label="Top-up minimum" value={money(rules.minimumTopUp)} tone="dark" /> : null}
                            <div className="grid grid-cols-2 gap-3">
                                <StatPill label={isOwnerWallet ? 'Fleet online' : 'Online earnings'} value={money(walletSummary.onlineRideEarnings)} tone="good" />
                                <StatPill label={isOwnerWallet ? 'Fleet cash commission' : 'Cash commission'} value={money(walletSummary.cashRideCommission)} tone="warn" />
                            </div>
                        </section>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-[11px] font-bold text-slate-500">
                                        Filter {isOwnerWallet ? 'fleet earnings and owner wallet entries' : 'earnings and wallet entries'} by type
                                    </p>
                                </div>
                                        ...
                                        >
                                            <div className="flex min-w-0 items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-sm font-black text-slate-950">{transactionLabel(tx.type, isOwnerWallet)}</p>
                                                    <p className="mt-0.5 text-xs font-bold leading-5 text-slate-500 break-words" title={transactionHint(tx, isOwnerWallet)}>
                                                        {shortenText(transactionHint(tx, isOwnerWallet))}
                                                    </p>
