...
const isDriverApproved = (driver) => {
    ...
    if (role === 'service_center' || role === 'service_center_staff' || role === 'bus_driver') {
...
const getPendingDriverRoute = (pathname = '', role = '') => {
    const normalizedRole = String(role || getAuthenticatedRole() || '').toLowerCase();
    ...
    return `${getPortalPrefix(pathname, normalizedRole === 'owner' ? 'owner' : 'driver')}/registration-status`;
};
        ...
        const verifyDriver = async () => {
            ...
            try {
                ...
                if (!isApproved) {
                    ...
                    verifiedTokenRef.current = '';
                    verifiedApprovalRef.current = false;
                    navigate(getPendingDriverRoute(currentPath, effectiveRole || authenticatedRole), { replace: true });
                    return;
                }
