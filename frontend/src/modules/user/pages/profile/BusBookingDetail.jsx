  ...
  useEffect(() => {
    ...
    const loadBooking = async () => {
      ...
      try {
        ...
        if (nextBooking?.canShowCheckInQr) {
          setLoadingQr(true);
          try {
            const qrResponse = await userBusService.getBookingCheckInQr(id);
            if (active) setCheckInQr(unwrapPayload(qrResponse));
          } catch {
            if (active) setCheckInQr(null);
          } finally {
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                {booking.canShowCheckInQr ? (
                  <div className="rounded-[28px] border border-slate-100 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-5 text-white shadow-sm">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 text-slate-300">
                          {isCheckedIn ? <CheckCircle2 size={16} className="text-emerald-300" /> : <QrCode size={16} />}
                          <p className="text-[10px] font-black uppercase tracking-[0.18em]">
                            {isCheckedIn ? 'Boarding Verified' : 'Boarding QR'}
                          </p>
                        </div>
                        <h3 className="mt-2 text-lg font-black">
                          {isCheckedIn ? 'Attendance marked' : 'Show this while boarding'}
                        </h3>
                        <p className="mt-1 text-xs font-semibold leading-5 text-slate-300">
                          The bus driver scans this QR to mark the passenger present for this ticket.
                        </p>
                      </div>
                      <div className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest ${
                        isCheckedIn ? 'bg-emerald-400 text-emerald-950' : 'bg-white/10 text-white'
                      }`}>
                        {isCheckedIn ? 'Checked In' : 'Pending'}
                      </div>
                    </div>

                    <div className="mt-5 rounded-[24px] bg-white p-4 text-center">
                      {loadingQr ? (
                        ...
                        <>
                          <img src={qrImageUrl} alt="Bus ticket boarding QR" className="mx-auto h-56 w-56 rounded-2xl" />
                          <p className="mt-3 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">
                            {booking.bookingCode}
                          </p>
                        </>
                      ) : (
                        <div className="flex min-h-[220px] flex-col items-center justify-center text-slate-500">
                          <CircleAlert size={24} />
                          <p className="mt-2 text-xs font-bold">QR is unavailable right now</p>
                        </div>
                      )}
                    </div>
                  </div>
