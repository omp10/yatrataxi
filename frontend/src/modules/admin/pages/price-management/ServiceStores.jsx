...
const ServiceStores = ({ mode: initialMode = 'list' }) => {
  ...
  const canUsePlacesAutocomplete = isLoaded && !loadError && Boolean(window.google?.maps?.places);
              ...
              <div className="space-y-6">
                <div className="rounded-[2.5rem] border border-slate-200 bg-white p-8 shadow-sm space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className={labelClass}>Owner Name</label><input type="text" value={formData.owner_name} onChange={e => setFormData(p => ({ ...p, owner_name: e.target.value }))} className={inputClass} placeholder="Owner name" /></div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className={labelClass}>Latitude</label><input type="text" value={formData.latitude} onChange={e => setFormData(p => ({ ...p, latitude: e.target.value }))} className={inputClass} placeholder="Tap map to fill" /></div>
                    <div><label className={labelClass}>Longitude</label><input type="text" value={formData.longitude} onChange={e => setFormData(p => ({ ...p, longitude: e.target.value }))} className={inputClass} placeholder="Tap map to fill" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-2.5 text-[11px] font-semibold leading-relaxed text-slate-500">{isResolvingAddress ? 'Resolving address from pinned point...' : 'Search above, drag the pin, or click directly on the map to set the store location.'}</div>
                  </div>
