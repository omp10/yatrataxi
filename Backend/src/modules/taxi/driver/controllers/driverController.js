...
import { verifyBusCheckInToken } from "../../services/busCheckInTokenService.js";
...
const serializeBusDriverBooking = (booking = {}) => ({
  ...
  checkIn: {
    status: booking.checkIn?.status || "pending",
    checkedInAt: booking.checkIn?.checkedInAt || null,
    checkedInByDriverId: booking.checkIn?.checkedInByDriverId
      ? String(booking.checkIn.checkedInByDriverId)
      : "",
    method: booking.checkIn?.method || "",
    lastScanAt: booking.checkIn?.lastScanAt || null,
  },
  routeSnapshot: booking.routeSnapshot || {},
  createdAt: booking.createdAt || null,
});
...
export const getActiveDriverRideRequest = async (req, res) => {
  ...
  const rideRequest = await getActiveRideRequestForDriver(driverId, rideId);
...
export const getMyWallet = async (req, res) => {
  if (String(req.auth?.role || "").toLowerCase() === "owner") {
    ...
    const ownerTransactions = await OwnerWalletTransaction.find({ ownerId: req.auth.sub })
      .sort({ createdAt: -1 })
      .limit(50)
      .lean();
    ...
    res.json({
      ...
      data: {
        ...
        transactions: ownerTransactions.map((item) => ({
          _id: item._id,
          amount: item.kind === "debit" ? -Math.abs(Number(item.amount || 0)) : Math.abs(Number(item.amount || 0)),
          type: item.metadata?.paymentMethod === "online" ? "ride_earning" : item.kind === "debit" ? "commission_deduction" : "adjustment",
...
const extractBusCheckInToken = (body = {}) => {
  const rawValue = toCleanString(body.token || body.qrValue || body.payload || body.code);
  if (!rawValue) {
    throw new ApiError(400, "Ticket QR payload is required");
  }

  try {
    const parsed = JSON.parse(rawValue);
    return toCleanString(parsed?.token || parsed?.qrToken || rawValue);
  } catch {
    return rawValue;
  }
};

const getActiveBusBookingSeatLabels = (booking = {}) => {
  const cancelledSeatIds = new Set(
    (Array.isArray(booking.cancelledSeats) ? booking.cancelledSeats : [])
      .map((seat) => toCleanString(seat?.seatId))
      .filter(Boolean),
  );

  const seatIds = Array.isArray(booking.seatIds) ? booking.seatIds : [];
  const seatLabels = Array.isArray(booking.seatLabels) ? booking.seatLabels : [];

  return seatIds
    .map((seatId, index) => ({
      seatId,
      seatLabel: seatLabels[index] || seatId,
    }))
    .filter((seat) => !cancelledSeatIds.has(toCleanString(seat.seatId)));
};

export const checkInBusTicketByQr = async (req, res) => {
  const busDriver = await BusDriver.findById(req.auth.sub).lean();
  ...
  const token = extractBusCheckInToken(req.body);
  const tokenPayload = verifyBusCheckInToken(token);

  if (!mongoose.Types.ObjectId.isValid(tokenPayload.bookingId)) {
    throw new ApiError(400, "Invalid ticket QR");
  }

  const booking = await BusBooking.findById(tokenPayload.bookingId);
  if (!booking) {
    throw new ApiError(404, "Bus booking not found");
  }

  if (String(booking.busServiceId || "") !== String(busDriver.assignedBusServiceId || "")) {
    throw new ApiError(403, "This ticket belongs to a different bus");
  }

  if (String(booking.status || "") !== "confirmed") {
    throw new ApiError(409, "Only confirmed tickets can be checked in");
  }

  if (Number(tokenPayload.tokenVersion || 1) !== Number(booking.checkIn?.tokenVersion || 1)) {
    throw new ApiError(401, "This ticket QR has been refreshed. Please scan the latest QR");
  }

  const activeSeats = getActiveBusBookingSeatLabels(booking);
  if (activeSeats.length <= 0) {
    throw new ApiError(409, "This ticket has no active seats");
  }

  const now = new Date();
  const alreadyCheckedIn = booking.checkIn?.status === "checked_in";

  booking.checkIn = {
    ...(booking.checkIn?.toObject ? booking.checkIn.toObject() : booking.checkIn || {}),
    status: "checked_in",
    checkedInAt: booking.checkIn?.checkedInAt || now,
    checkedInByDriverId: booking.checkIn?.checkedInByDriverId || busDriver._id,
    method: booking.checkIn?.method || "qr",
  ...
  res.json({
    success: true,
    message: alreadyCheckedIn ? "Passenger was already checked in" : "Passenger checked in successfully",
    data: {
      alreadyCheckedIn,
      booking: serializeBusDriverBooking(booking),
      passenger: booking.passenger || {},
      seats: activeSeats,
      route: booking.routeSnapshot || {},
      checkedInAt: booking.checkIn.checkedInAt,
      lastScanAt: booking.checkIn.lastScanAt,
    },
  });
};
...
export const getDriverApprovalStatus = async (req, res) => {
  ...
  if (normalizedRole === "service_center_staff") {
    const access = await resolveAuthenticatedServiceCenterAccess({
      auth: {
        sub: payload.sub,
        role: normalizedRole,
      },
  ...
  if (normalizedRole === "service_center") {
    const center = await resolveAuthenticatedServiceCenter({
      auth: {
        sub: payload.sub,
        role: normalizedRole,
      },
