...
const resolveOwnerCommissionConfigForRide = async (ride, session) => {
  if (ride?.pricingSnapshot?.admin_commission_for_owner !== undefined) {
    return {
      source: ride.pricingSnapshot?.setPriceId ? 'ride_snapshot_owner' : 'ride_snapshot_owner_fallback',
      type: Number(ride.pricingSnapshot?.admin_commission_type_for_owner ?? 1),
      value: Number(ride.pricingSnapshot?.admin_commission_for_owner ?? 0),
  ...
  if (ride?.vehicleTypeId) {
    ...
    for (const filter of filters) {
      ...
      if (setPrice) {
        return {
          source: 'set_price_owner_lookup',
          type: Number(setPrice.admin_commission_type_for_owner ?? 1),
          value: Number(setPrice.admin_commission_for_owner ?? 0),
          setPriceId: setPrice._id,
        };
      }
    }
...
const applyOwnerWalletAdjustment = async ({
  ownerId,
  amount,
  rideId = null,
  title = '',
  ...
  if (!normalizedAmount) {
    throw new ApiError(400, 'Owner wallet adjustment amount cannot be zero');
  }
  ...
  const balanceAfter = Math.round((balanceBefore + normalizedAmount) * 100) / 100;
  ...
  const [transaction] = await OwnerWalletTransaction.create(
    [
      {
        ownerId,
        rideId,
        amount: Math.abs(normalizedAmount),
        kind: normalizedAmount < 0 ? 'debit' : 'credit',
        title,
  ...
  return {
    owner: updatedOwner,
    wallet: {
      balance: Number(updatedOwner.wallet?.balance || 0),
      currency: 'INR',
    },
    transaction,
  };
...
export const settleCompletedRideWallet = async ({ rideId }) => {
  ...
  try {
    ...
    const ownerId = driver?.owner_id || null;
    const fare = normalizeAmount(ride.fare || 0, 'fare');
    const commissionConfig = ownerId
      ? await resolveOwnerCommissionConfigForRide(ride, session)
      : await resolveCommissionConfigForRide(ride, session);
    ...
    ride.pricingSnapshot = {
      ...
      admin_commission_type_for_owner: ownerId
        ? Number(commissionConfig.type ?? ride.pricingSnapshot?.admin_commission_type_for_owner ?? 1)
        : Number(ride.pricingSnapshot?.admin_commission_type_for_owner ?? 1),
      admin_commission_for_owner: ownerId
        ? Number(commissionConfig.value ?? ride.pricingSnapshot?.admin_commission_for_owner ?? 0)
        : Number(ride.pricingSnapshot?.admin_commission_for_owner ?? 0),
    ...
    const result = ownerId
      ? await applyOwnerWalletAdjustment({
        ownerId,
        ...adjustmentPayload,
        title: paymentMethod === 'cash'
          ? 'Commission deducted for fleet cash ride'
          : 'Fleet ride earning credited for online ride',
      })
