...
const syncDefaultAdminRecord = async () => {
  ...
  let nextPassword;
  if (!existingAdmin) {
    nextPassword = await hashPassword(DEFAULT_ADMIN_PASSWORD);
  } else if (!BCRYPT_HASH_PATTERN.test(existingAdmin.password || '')) {
    nextPassword = await hashPassword(DEFAULT_ADMIN_PASSWORD);
  } else {
    const isMatch = await comparePassword(DEFAULT_ADMIN_PASSWORD, existingAdmin.password);
    if (!isMatch) {
      nextPassword = await hashPassword(DEFAULT_ADMIN_PASSWORD);
    }
  }
...
const seedInitialData = async () => {
  ...
  if (await Agent.countDocuments() === 0) {
    const agentPasswordHash = await hashPassword('agent-7223077890');
    await Agent.create({
      phone: '7223077890',
      countryCode: '+91',
      password: agentPasswordHash,
      name: 'Seed Agent 7223077890',
      email: 'agent.7223077890@Yatra Desk.local',
      active: true,
      status: 'active',
      kycStatus: 'verified',
...
export const forgotPassword = async (email) => {
  ...
  await sendEmail({
    to: email,
    subject: `Password Reset OTP for ${process.env.APP_NAME || 'Yatra Desk'}`,
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px;">
        ...
        <p style="font-size: 12px; color: #666;">Regards,<br>Team ${process.env.APP_NAME || 'Yatra Desk'}</p>
      </div>
    `,
  });
