...
import { signBusCheckInToken } from '../../services/busCheckInTokenService.js';
...
const serializeBusBooking = (booking, busService = null) => {
  ...
  canShowCheckInQr: String(booking.status || '') === 'confirmed' && activeSeatCount > 0,
...
const buildReactivatedUserPayload = async ({ req, name, phone, email, countryCode, gender, profileImage, referrer, referrerAgent }) => ({
  name,
  phone,
  countryCode,
  email,
...
const findAgentByReferralCode = async (referralCode) => {
  ...
  return Agent.findOne({ referralCode: normalizedCode });
};
...
export const registerUser = async (req, res) => {
  ...
  const referrerAgent = !referrer && referralCode ? await findAgentByReferralCode(referralCode) : null;

  if (referralCode && !referrer && !referrerAgent) {
  ...
  if (referrerAgent?._id) {
    await Agent.updateOne({ _id: referrerAgent._id }, { $inc: { 'metrics.totalCustomers': 1 } });
  }
...
export const getMyBusBookingCheckInQr = async (req, res) => {
  await ensureBusServiceEnabled();
  await cleanupExpiredBusSeatHolds();
  ...
  const activeSeatCount = (Array.isArray(booking.seatIds) ? booking.seatIds : [])
    .filter((seatId) => !cancelledSeatIds.has(toCleanString(seatId))).length;

  if (String(booking.status || '') !== 'confirmed' || activeSeatCount <= 0) {
    throw new ApiError(409, 'Only confirmed active bus tickets can generate a boarding QR');
  }

  const token = signBusCheckInToken(booking);
  const qrValue = JSON.stringify({
    type: 'bus_ticket_checkin',
    token,
  });
  ...
  res.status(200).json({
    ...
    data: {
      ...
      passenger: booking.passenger || {},
      seats: {
        labels: Array.isArray(booking.seatLabels) ? booking.seatLabels : [],
        activeCount: activeSeatCount,
      },
      expiresInDays: 30,
    },
  });
