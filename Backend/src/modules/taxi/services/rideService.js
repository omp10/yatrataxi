...
export const updateRideLifecycle = async ({ rideId, driverId, nextStatus, paymentMethod }) => {
  ...
  if (nextStatus === RIDE_LIVE_STATUS.COMPLETED) {
    ...
    const user = await User.findById(ride.userId).select('referredByAgent').lean();
    const resolvedAgentId = ride.agentMeta?.bookedByAgentId || user?.referredByAgent || null;
    const commissionMode = ride.agentMeta?.bookedByAgentId ? 'direct' : (user?.referredByAgent ? 'referral' : '');
    const bookingType = String(ride.serviceType || '').trim().toLowerCase() === 'intercity' ? 'intercity' : 'ride';

    if (resolvedAgentId) {
      const agentCommission = await creditAgentCommission({
        agentId: resolvedAgentId,
        bookingType,
        commissionMode,
        grossAmount: ride.fare || 0,
        referenceKey: `agent:${bookingType}:${String(ride._id)}`,
        title: bookingType === 'intercity'
          ? `Intercity booking commission for ride ${String(ride._id).slice(-6)}`
          : `Ride booking commission for ride ${String(ride._id).slice(-6)}`,
        metadata: {
          rideId: String(ride._id),
      ...
      if (agentCommission?.transaction) {
        ride.agentMeta = {
          ...(ride.agentMeta || {}),
